...
export type ApprovalArchiveSource = Record<string, unknown>;
...
export function serializeFormFields(metaData: Record<string, unknown>): string {
  const lines: string[] = [];
...
export function formatApprovalArchiveDate(value: unknown) {
  const raw = String(value || '').trim();
  if (!raw) return '-';
  return formatKoreanDateTime(raw) || raw;
}

export function resolveApprovalDocNumber(item: ApprovalArchiveSource) {
  const metaData = safeParseMetaData(item.meta_data);
  return String(item.doc_number || metaData.doc_number || '').trim();
}

export function resolveApprovalCategory(item: ApprovalArchiveSource) {
...
export function buildApprovalArchiveContent(item: ApprovalArchiveSource) {
  const metaData = safeParseMetaData(item.meta_data);
...
export function buildApprovalArchiveId(approvalId: unknown) {
