...
(function () {
  ...
  const DEVELOPER = window.EMO_DEVELOPER;
  const byId = {};
  [...OFFICE, ...HOSPITAL, ...DEVELOPER].forEach(d => byId[d.id] = d);
  ...
  const ROOMS = {
    ...
    developer: {
      name: "IT 개발실 단톡방", sub: "팀원 5", color: "hospital",
      botName: "수석아키텍트", botInit: "아", botColor: "#5cb89f",
      set: DEVELOPER,
      seed: [
        { who: "bot", text: "오늘 배포 예정대로 진행하나요?" },
        { who: "bot", emo: "d-coding" },
        { who: "me", emo: "d-turtle" },
      ],
      replies: [
        "LGTM!! 바로 배포 준비하겠습니다.", "서버 무중단 배포 완료!", "에러 로그 확인 중입니다 ㅠㅠ",
        "커피 수혈 시급하네요...", "컴파일 왜 이렇게 오래 걸리죠..", "코드 정리하고 올게요!",
      ],
      replyEmos: ["d-lgtm","d-release","d-caffeine","d-headphones","d-gitpush","d-cleanup"],
    },
  };

  let current = "office";
  const history = { office: [], hospital: [], developer: [] };
  ...
  function buildGallery() {
    ...
    grid.innerHTML = "";
    [["직장인","office",OFFICE],["의료진","hospital",HOSPITAL],["IT 개발자","hospital",DEVELOPER]].forEach(([title,color,set]) => {
