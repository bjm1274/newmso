 ...
 * - ESC 키 / 오버레이 클릭으로 닫힘
 * - 포커스 트랩 (useModalFocusTrap 재사용)
 * - JM6: role="dialog", aria-modal, aria-labelledby, 포커스 복귀
 * - JM2: useMemo로 검색 결과 캐싱, 불필요한 리렌더 방지
 * - JM4: any 금지, MoreSheetProps 명시
 *
 * ⚠️ 경로 alias 주의:
 *   이 파일은 analysis_artifacts/ 아래에 위치하므로 tsconfig include 범위 밖.
 *   app/main-v2/ 로 이동 후에는 아래 import 경로를 "@/" 형태로 수정하세요.
 *   (현재는 import 경로를 문자열 주석으로만 기록하고, 런타임 복사본 기준으로 작성)
 */

import React, { useCallback, useEffect, useMemo, useRef, useState } from 'react';
import { Search, X, ChevronRight } from 'lucide-react';

// ── 의존 import (app/main-v2/ 이동 후 경로 교체 필요) ─────────────────────────
