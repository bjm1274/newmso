...
function buildObjectKey(company: string, fileName: string, mimeType: string): string {
  ...
  return `logos/${safeFolder}_${Date.now()}_${crypto.randomUUID()}.${ext}`;
}

/**
 * 회사(병원) 로고 업로드 — 재직증명서 헤더/워터마크용.
 * 직인(/api/admin/seal/upload)과 동일한 인증·R2 패턴.
 */
export async function POST(request: Request): Promise<NextResponse> {
  try {
    ...
    if (contentLength > MAX_BYTES) {
      return NextResponse.json({ error: '로고 이미지는 5MB 이하여야 합니다.' }, { status: 413 });
    }
    ...
    const message = error instanceof Error ? error.message : '로고 업로드 중 오류가 발생했습니다.';
