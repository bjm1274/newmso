    ...
    <div className="bg-[var(--toss-card)] border border-[var(--toss-border)] rounded-3xl overflow-hidden shadow-sm animate-in fade-in duration-300">
      ...
      {formType === '연차/휴가' && (
        <>
          ...
          <div className="p-4 md:p-6 grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 bg-gray-50/30 items-start">
            <div className="space-y-2">
              <label className="text-[11px] font-bold text-[var(--toss-blue)] ml-1 uppercase">휴가 종류</label>
              <select data-testid="approval-leave-type-select" className="w-full h-[52px] px-4 rounded-[12px] bg-[var(--toss-card)] font-bold text-xs border border-[var(--toss-border)] shadow-sm focus:ring-2 focus:ring-[var(--toss-blue)]/30"
            ...
            <div className="space-y-2">
              ...
              <SmartDatePicker data-testid="approval-leave-start-date" value={localStartDate} onChange={val => { setLocalStartDate(val); setExtraData((prev: any) => ({ ...prev, startDate: val })); }} className="w-full" inputClassName="h-[52px] rounded-[12px] border border-[var(--toss-border)] bg-[var(--toss-card)] px-4 font-bold text-xs shadow-sm focus:ring-2 focus:ring-[var(--toss-blue)]/30" />
            </div>
            <div className="space-y-2">
              <label className="text-[11px] font-bold text-[var(--toss-blue)] ml-1 uppercase">종료 일자</label>
              <SmartDatePicker data-testid="approval-leave-end-date" value={localEndDate} onChange={val => { setLocalEndDate(val); setExtraData((prev: any) => ({ ...prev, endDate: val })); }} className="w-full" inputClassName="h-[52px] rounded-[12px] border border-[var(--toss-border)] bg-[var(--toss-card)] px-4 font-bold text-xs shadow-sm focus:ring-2 focus:ring-[var(--toss-blue)]/30" />
            </div>
          </div>
        </>
      )}
