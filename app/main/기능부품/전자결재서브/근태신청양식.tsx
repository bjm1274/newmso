          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-h-60 overflow-y-auto pr-2 custom-scrollbar">
            {attData.map((a, i) => {
              ...
              return (
                ...
                }} className={`p-5 rounded-[16px] border-2 text-left transition-all flex justify-between items-center ${selectedDate === a.date ? 'border-orange-500 bg-[var(--toss-card)] shadow-lg' : 'bg-[var(--toss-card)]/50 border-[var(--toss-border)] hover:bg-[var(--toss-card)]'}`}>
                  <div><span className="text-[11px] font-bold text-[var(--toss-gray-3)]">{a.date}</span><p className="text-xs font-bold text-[var(--foreground)]">퇴근: {a.check_out.slice(11,16)}</p></div>
                  <span className="text-[11px] font-bold text-orange-500 bg-orange-50 px-2 py-1 rounded-[12px]">+{ot}H</span>
                </button>
              );
            })}
          </div>
