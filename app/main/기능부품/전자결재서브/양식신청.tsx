...
export default function FormRequest({
  ...
  const handleSubmit = async () => {
    ...
    if (!selectedForm) {
      toast('발급할 증명서를 선택해 주세요.', 'warning');
      return;
    }
    ...
    try {
      ...
      const payload = {
        ...
        // 결재 type 라벨을 '증명서발급'으로 통일. 기존 '양식신청' 레코드는 조회 측에서 호환 처리.
        type: '증명서발급',
        title: `${formLabel} 발급 신청`,
      ...
      toast('증명서 발급 신청이 완료되었습니다. 결재자의 확인을 기다려 주세요.', 'success');
      setPurpose('');
      setUrgency('일반');
      setSelectedForm(forms[0]?.id ?? '');
    } catch (error) {
      console.error('증명서 발급 신청 실패:', describeError(error));
      toast('증명서 발급 신청 중 오류가 발생했습니다.', 'error');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    ...
    >
      <header className="border-b pb-3">
        <h3 className="text-lg font-semibold tracking-tight text-[var(--foreground)]">증명서 발급</h3>
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <textarea
            ...
            placeholder="발급 용도를 입력해 주세요. 예: 금융기관 제출용, 사직 확인용"
        ...
        {approverLine.length === 0 && (
          <div className="rounded-[var(--radius-md)] border border-red-500/20 bg-red-500/10 px-3 py-2 text-[11px] font-bold text-red-600" data-testid="form-request-approver-required">
            결재권자를 먼저 선택해야 증명서 발급 신청을 올릴 수 있습니다.
          </div>
        )}
      ...
      <div className="space-y-1.5 rounded-[var(--radius-md)] border border-[var(--accent)]/30 bg-[var(--toss-blue-light)] p-3">
        ...
        <p className="text-xs font-bold leading-relaxed text-[var(--accent)]">
          최종 결재가 승인되면 선택한 증명서가 자동으로 생성되어 행정팀에서 발급 처리됩니다.
        </p>
      </div>
    </div>
  );
