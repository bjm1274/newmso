      ...
      <div className="border border-gray-200 p-6 bg-white rounded-2xl">
        ...
        {certs.length === 0 ? <p className="text-xs text-gray-500">등록된 자격이 없습니다.</p> : (
          <div className="space-y-2">
            {certs.map((x) => (
              <div key={x.id} className="p-3 bg-gray-50 rounded-lg">
                <p className="text-sm font-bold">{x.name}</p>
                <p className="text-[10px] text-gray-500">{x.issuer} · {x.issue_date} {x.expiry_date ? `~ ${x.expiry_date}` : ''}</p>
              </div>
            ))}
          </div>
        )}
          ...
          <div className="space-y-2">
            {transfers.map((x) => (
              <div key={x.id} className="p-3 bg-gray-50 rounded-lg">
                ...
                <p className="text-[10px] text-gray-500">{x.before_value} → {x.after_value} ({x.effective_date})</p>
              </div>
            ))}
          </div>
        )}
