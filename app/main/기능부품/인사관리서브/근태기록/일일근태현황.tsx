    ...
    <div className="bg-white rounded-[2rem] border border-gray-100 shadow-sm overflow-hidden h-full">
      ...
      <div className="overflow-y-auto h-full pb-20 custom-scrollbar">
        <table className="w-full text-sm text-left">
            <thead className="bg-gray-50 text-gray-500 font-bold sticky top-0"><tr><th className="p-4">이름</th><th className="p-4">출근</th><th className="p-4">퇴근</th><th className="p-4">상태</th><th className="p-4">관리</th></tr></thead>
            <tbody>
            {dailyData.map((d: any, idx: number) => (
                ...
                <td className="p-4 text-blue-600">{d.check_in ? d.check_in.slice(11,16) : '-'}</td>
                ...
                <td className="p-4"><button onClick={()=>onEdit(d, d.staff)} className="text-xs bg-gray-100 px-2 py-1 rounded font-bold hover:bg-gray-200">수정</button></td>
                </tr>
            ))}
            </tbody>
        </table>
