...
export default function DailyView({ date, dailyData, onEdit }: Record<string, unknown>) {
  const _dailyData = (dailyData as any[]) ?? [];
  const _onEdit = onEdit as (...args: any[]) => void;
  return (
    <div className="bg-[var(--card)] rounded-[var(--radius-lg)] border border-[var(--border)] shadow-sm overflow-hidden h-full">
      ...
      <div className="overflow-y-auto h-full pb-20 custom-scrollbar">
        ...
        <table className="w-full text-sm text-left">
            ...
            <tbody>
            {_dailyData.map((d: any, idx: number) => (
                ...
                <td className="p-4"><button onClick={()=>_onEdit(d, d.staff)} className="text-xs bg-[var(--muted)] px-2 py-1 rounded font-bold hover:bg-[var(--toss-gray-2)]">수정</button></td>
                </tr>
            ))}
            </tbody>
        </table>
