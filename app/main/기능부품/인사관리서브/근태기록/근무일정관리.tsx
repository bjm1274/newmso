  ...
  return (
    <div className="bg-[var(--card)] rounded-[var(--radius-lg)] border border-[var(--border)] shadow-sm p-4 h-full overflow-y-auto custom-scrollbar">
        <div className="flex justify-between items-end mb-4 border-b pb-4">
            ...
            <button onClick={onAdd} className="bg-black text-white px-5 py-3 rounded-[var(--radius-md)] text-sm font-bold shadow-sm hover:scale-105 transition-transform flex items-center gap-2">
            ...
            {schedules.map((s: any) => (
                <div key={s.id} className="border border-[var(--border)] p-4 rounded-[var(--radius-lg)] bg-[var(--card)] hover:shadow-sm hover:border-blue-500 transition-all group relative flex flex-col justify-between min-h-[200px]">
                    ...
                    <div className="flex gap-2 mt-4 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button onClick={() => onEdit(s)} className="flex-1 py-2.5 bg-[var(--muted)] text-[var(--toss-gray-4)] rounded-[var(--radius-lg)] text-xs font-bold hover:bg-[var(--toss-gray-2)]">수정</button>
                        <button onClick={() => onDelete(s.id)} className="flex-1 py-2.5 bg-red-50 text-red-500 rounded-[var(--radius-lg)] text-xs font-bold hover:bg-red-100">삭제</button>
                    </div>
                </div>
            ))}
            
            {/* 추가 안내 카드 */}
            <button onClick={onAdd} className="border-2 border-dashed border-[var(--border)] p-4 rounded-[var(--radius-lg)] flex flex-col items-center justify-center text-center text-[var(--toss-gray-3)] hover:border-[var(--border)] hover:text-[var(--toss-gray-4)] transition-all min-h-[200px]">
