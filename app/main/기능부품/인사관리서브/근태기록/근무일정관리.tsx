...
export default function ScheduleManager({ schedules, onAdd, onEdit, onDelete }: any) {
  return (
    <div className="bg-white rounded-[2rem] border border-[var(--toss-border)] shadow-sm p-8 h-full overflow-y-auto custom-scrollbar">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            {schedules.map((s: any) => (
                <div key={s.id} className="border border-[var(--toss-border)] p-6 rounded-[2rem] bg-white hover:shadow-xl hover:border-blue-500 transition-all group relative flex flex-col justify-between min-h-[200px]">
                    <div>
                        <div className="flex justify-between items-start mb-2">
                            <div className="flex items-center gap-2">
                                <h4 className="font-bold text-lg text-[var(--foreground)]">{s.name}</h4>
                                {s.is_default && <span className="bg-[var(--toss-blue-light)] text-[var(--toss-blue)] text-[10px] px-2 py-1 rounded-full font-bold">기본값</span>}
                            </div>
                        ...
                        {/* 시간 정보 그리드 */}
                        <div className="bg-[var(--toss-gray-1)] rounded-xl p-3 space-y-2">
                    ...
                    <div className="flex gap-2 mt-6 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button onClick={() => onEdit(s)} className="flex-1 py-2.5 bg-[var(--toss-gray-1)] text-[var(--toss-gray-4)] rounded-xl text-xs font-bold hover:bg-[var(--toss-gray-2)]">수정</button>
            ...
            {/* 추가 안내 카드 */}
            <button onClick={onAdd} className="border-2 border-dashed border-[var(--toss-border)] p-6 rounded-[2rem] flex flex-col items-center justify-center text-center text-[var(--toss-gray-3)] hover:border-[var(--toss-border)] hover:text-[var(--toss-gray-4)] transition-all min-h-[200px]">
