    ...
    <div className="p-4 md:p-6 space-y-6">
      ...
      <div className="bg-[var(--toss-card)] border border-[var(--toss-border)] rounded-2xl p-4">
        ...
        <div className="flex flex-wrap gap-2">
          {filtered.map((s: any) => {
            ...
            return (
              <div key={s.id} className="flex items-center gap-2 px-3 py-2 bg-[var(--toss-gray-1)] rounded-xl border border-[var(--toss-border)]">
                <span className="text-xs font-bold text-[var(--foreground)]">{s.name}</span>
                <span className={`px-2 py-0.5 text-[10px] font-extrabold rounded-lg ${TYPE_COLOR[wt]}`}>{wt}</span>
              </div>
            );
          })}
      ...
      {showForm && (
        <div className="bg-blue-50 border border-[var(--toss-blue)]/30 rounded-2xl p-5 space-y-4">
          <h3 className="text-sm font-bold text-[var(--toss-blue)]">근무 형태 변경 이력 등록</h3>
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => setShowForm(false)} className="px-5 py-2.5 bg-[var(--toss-gray-1)] text-[var(--foreground)] text-xs font-bold rounded-xl hover:bg-[var(--toss-gray-2)] transition-colors">
              취소
            </button>
          </div>
        </div>
      ...
      <div className="flex items-center gap-3">
        <input
          ...
          placeholder="직원명 검색..."
          className="px-4 py-2 text-sm border border-[var(--toss-border)] rounded-xl bg-[var(--toss-card)] text-[var(--foreground)] outline-none focus:ring-2 focus:ring-[var(--toss-blue)]/30 w-56"
        />
        {searchName && (
      ...
      <div className="bg-[var(--toss-card)] border border-[var(--toss-border)] rounded-2xl overflow-hidden">
        ...
        {loading ? (
          ...
          <div className="p-10 text-center text-sm text-[var(--toss-gray-3)]">변경 이력이 없습니다.</div>
        ) : (
            ...
            <table className="w-full text-xs">
              ...
              <tbody className="divide-y divide-[var(--toss-border)]">
                {displayRecords.map((rec) => (
                  <tr key={rec.id} className="hover:bg-[var(--toss-gray-1)]/50 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      {rec.prev_type ? (
                        <span className={`px-2 py-0.5 rounded-lg text-[10px] font-extrabold ${TYPE_COLOR[rec.prev_type as WorkType] || 'bg-gray-100 text-gray-600'}`}>{rec.prev_type}</span>
                      ) : <span className="text-[var(--toss-gray-3)]">-</span>}
                    </td>
                    <td className="px-4 py-3">
                      <span className={`px-2 py-0.5 rounded-lg text-[10px] font-extrabold ${TYPE_COLOR[rec.new_type as WorkType] || 'bg-gray-100 text-gray-600'}`}>{rec.new_type}</span>
                    </td>
