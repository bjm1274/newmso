  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div className="bg-white border border-[var(--toss-border)] p-6 shadow-sm flex flex-col justify-between">
        <div>
          <p className="text-[10px] font-semibold text-[var(--toss-gray-3)] uppercase mb-1 tracking-widest">평균 이수율</p>
      ...
      <div className={`bg-white border p-6 shadow-sm transition-all ${urgentCount > 0 ? 'border-red-200 bg-red-50/30' : 'border-[var(--toss-border)]'}`}>
        <p className="text-[10px] font-semibold text-[var(--toss-gray-3)] uppercase mb-1 tracking-widest">미이수 인원</p>
        <div className="flex items-center justify-between mt-4">
          ...
          <button className="text-[10px] font-semibold text-[var(--toss-blue)] border border-blue-100 px-3 py-1.5 bg-white hover:bg-blue-50 transition-all shadow-sm">
            전체 명단 확인
          </button>
        </div>
        <p className="text-[9px] text-[var(--toss-gray-3)] font-bold mt-4 italic">* 법정 의무 교육 미이수 시 과태료 리스크</p>
      </div>

      {/* 3. 집중 관리 항목 (병원 필수 교육) */}
      <div className="bg-[#232933] p-6 shadow-xl flex flex-col">
        <p className="text-[10px] font-semibold text-[var(--toss-gray-3)] uppercase mb-4 tracking-widest">집중 관리 교육</p>
        ...
        <p className="text-[10px] text-[var(--toss-gray-3)] font-bold mt-auto pt-4 border-t border-white/5">
          의료진 및 행정직 필수 이수 항목
        </p>
      </div>
    </div>
