                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
                    {summaryCards.map(card => (
                        <div key={card.name} className="bg-[var(--card)] border border-[var(--border)] rounded-2xl p-4 shadow-sm">
                            <p className="text-[11px] font-bold text-[var(--toss-gray-3)] uppercase tracking-wider mb-2">{card.name}</p>
                            ...
                            {card.pending > 0 && (
                                <p className="text-[11px] font-bold text-amber-600 mt-1">미신고 {card.pending}건</p>
                            )}
                        </div>
                    ))}
                </div>
                    ...
                    <form onSubmit={handleSubmit} className="bg-[var(--card)] border border-[var(--border)] rounded-2xl p-4 shadow-sm space-y-4 animate-in slide-in-from-top-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div className="flex flex-col gap-1.5">
                                <label className="text-[11px] font-bold text-[var(--toss-gray-4)]">대상 직원</label>
                            ...
                            <div className="flex flex-col gap-1.5">
                                <label className="text-[11px] font-bold text-[var(--toss-gray-4)]">신고 유형</label>
                            ...
                            <div className="flex flex-col gap-1.5">
                                <label className="text-[11px] font-bold text-[var(--toss-gray-4)]">보험 종류</label>
                            ...
                            <div className="flex flex-col gap-1.5">
                                <label className="text-[11px] font-bold text-[var(--toss-gray-4)]">적용일</label>
                            ...
                            <div className="flex flex-col gap-1.5">
                                <label className="text-[11px] font-bold text-[var(--toss-gray-4)]">사유</label>
                            ...
                            <div className="flex flex-col gap-1.5">
                                <label className="text-[11px] font-bold text-[var(--toss-gray-4)]">비고</label>
                ...
                <div className="flex flex-wrap items-center gap-2">
                    <div className="flex gap-1 bg-[var(--card)] border border-[var(--border)] rounded-xl p-1">
                        {['전체', ...ACTION_TYPES].map(t => (
                            <button key={t} onClick={() => setFilter(t)} className={`px-3 py-1.5 text-[11px] font-bold rounded-lg transition-all ${filter === t ? 'bg-[var(--accent)] text-white shadow-sm' : 'text-[var(--toss-gray-3)] hover:text-[var(--foreground)]'}`}>{t}</button>
                        ))}
                    </div>
                    <div className="flex gap-1 bg-[var(--card)] border border-[var(--border)] rounded-xl p-1">
                        {['전체', '미신고', '신고완료', '반려'].map(s => (
                            <button key={s} onClick={() => setStatusFilter(s)} className={`px-3 py-1.5 text-[11px] font-bold rounded-lg transition-all ${statusFilter === s ? 'bg-[var(--foreground)] text-white shadow-sm' : 'text-[var(--toss-gray-3)] hover:text-[var(--foreground)]'}`}>{s}</button>
                        ))}
                    </div>
                    <input
                        type="text"
                            ...
                            <div key={s.id} className="flex items-center justify-between p-3 bg-blue-50 border border-blue-200 rounded-xl">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-[11px] text-[var(--toss-gray-3)]">입사일: {s.joined_at} · 4대보험 취득 신고 필요</p>
                                    </div>
                                </div>
                                <button
                                    onClick={() => {
                                    ...
                                    className="px-3 py-1.5 bg-[var(--accent)] text-white text-[11px] font-bold rounded-lg hover:opacity-90 transition-all"
                                >
                                    취득 신고
                                </button>
                            </div>
                            ...
                            <div key={s.id} className="flex items-center justify-between p-3 bg-red-50 border border-red-200 rounded-xl">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-[11px] text-[var(--toss-gray-3)]">퇴사일: {s.resigned_at} · 4대보험 상실 신고 확인 필요</p>
                                    </div>
                                </div>
                                <button
                                    onClick={() => {
