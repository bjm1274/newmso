...
export default function TaxReporter({ employees }: { employees?: any[] }) {
  return (
    <div className="border border-[var(--toss-border)] p-4 bg-[var(--toss-card)] rounded-[12px] shadow-sm">
      ...
      <button className="w-full py-2.5 bg-[var(--page-bg)] border border-[var(--toss-border)] rounded-[12px] text-xs font-medium text-[var(--toss-blue)] hover:bg-[var(--toss-blue-light)]">
        국세청 신고 데이터 추출 (SAM)
      </button>
    </div>
  );
