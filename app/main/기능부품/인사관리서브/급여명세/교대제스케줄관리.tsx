  ...
  return (
    <div className="border border-[var(--toss-border)] p-4 bg-[var(--toss-card)] rounded-lg shadow-sm">
      ...
      <div className="space-y-2">
        {shifts.map((s) => (
          <div key={s.id} className="p-3 bg-[var(--page-bg)] rounded-lg border border-[var(--toss-border)] flex justify-between items-center">
            <div>
              <p className="text-sm font-semibold text-[var(--foreground)]">{s.name}</p>
              <p className="text-[10px] text-[var(--toss-gray-3)]">{s.start_time} ~ {s.end_time}</p>
            </div>
            {s.shift_type && <span className="text-xs font-medium text-[var(--toss-blue)]">{s.shift_type}</span>}
          </div>
        ))}
      </div>
      <p className="mt-3 text-[10px] text-[var(--toss-gray-3)]">* 근무형태 관리에서 교대 유형(day/swing/night)을 설정할 수 있습니다.</p>
    </div>
  );
}
