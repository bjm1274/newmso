    ...
    <div className="p-4 md:p-6 space-y-6">
        ...
        <div className="flex flex-wrap gap-2">
          {filtered.map((s: any) => (
            <button
              key={s.id}
              onClick={() => openAdd(s)}
              className="px-3 py-1.5 text-xs font-bold bg-[var(--toss-gray-1)] hover:bg-[var(--toss-blue)] hover:text-white text-[var(--foreground)] rounded-xl border border-[var(--toss-border)] transition-all"
            >
              + {s.name}
            </button>
          ))}
      ...
      {editingId !== null && (
        <div className="bg-blue-50 border border-[var(--toss-blue)]/30 rounded-2xl p-5 space-y-4">
          <h3 className="text-sm font-bold text-[var(--toss-blue)]">{editingId > 0 ? '결근 기록 수정' : '결근 기록 추가'} — {form.staff_name}</h3>
          ...
          <div className="flex items-center gap-4 bg-white rounded-xl border border-[var(--toss-border)] px-4 py-3">
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              className="px-5 py-2.5 bg-[var(--toss-gray-1)] text-[var(--foreground)] text-xs font-bold rounded-xl hover:bg-[var(--toss-gray-2)] transition-colors"
            >
              취소
            </button>
          </div>
      ...
      <div className="bg-[var(--toss-card)] border border-[var(--toss-border)] rounded-2xl overflow-hidden">
        ...
        {loading ? (
          ...
          <div className="p-10 text-center text-sm text-[var(--toss-gray-3)]">해당 월 결근 기록이 없습니다.</div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
                  ...
                  <tr key={rec.id} className="hover:bg-[var(--toss-gray-1)]/50 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex gap-1">
                        <button onClick={() => openEdit(rec)} className="px-2 py-1 text-[10px] font-bold bg-blue-50 text-[var(--toss-blue)] rounded-lg hover:bg-blue-100 transition-colors">수정</button>
              ...
              <tfoot className="bg-[var(--toss-gray-1)] border-t-2 border-[var(--toss-border)]">
                <tr>
