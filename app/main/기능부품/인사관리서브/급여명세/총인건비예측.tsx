  ...
  useEffect(() => {
    const fetch12Months = async () => {
      ...
      try {
        ...
        // 중간정산(interim) 데이터 필터링을 통해 정규 급여 데이터만 집계
        const records = filterNonInterimPayrollRecords((data || []) as any[]);
        ...
        console.error('fetch12Months error in TotalLaborCostForecast:', error);
        setMonthlyData([]);
      } finally {
        setLoading(false);
      }
