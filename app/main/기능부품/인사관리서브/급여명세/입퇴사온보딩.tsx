...
export default function OnboardingChecklist({ staffId, staffName, type }: { staffId: string; staffName: string; type: '입사' | '퇴사' }) {
  ...
  return (
    <div className="app-card p-4 rounded-[var(--radius-md)] shadow-sm">
      <div className="flex justify-between items-center pb-2 border-b border-[var(--border)] mb-3">
        ...
        <span className="text-xs font-semibold text-[var(--accent)]">{doneCount}/{totalCount} 완료</span>
      </div>
      ...
      <div className="space-y-1">
        {items.map((x, i) => (
          <label key={i} className="flex items-center gap-3 py-2 px-3 rounded-[var(--radius-md)] cursor-pointer hover:bg-[var(--page-bg)] transition-colors border border-transparent hover:border-[var(--border)]">
            <input type="checkbox" checked={x.done} onChange={() => toggle(i)} className="w-4 h-4 rounded border-[var(--border)]" />
