...
export default function OrgChartEditor({
  ...
  const importFromStaff = async () => {
    ...
    try {
      ...
      const staffRows = sourceStaffs.map(s => ({
      ...
      {loading ? (
        ...
        <div className="flex-1 overflow-auto p-4">
          <div className="space-y-2">
            {nodes.map(n => (
              <div key={n.id} className="flex items-center justify-between p-3 bg-[var(--card)] border border-[var(--border)] rounded-[var(--radius-md)]">
