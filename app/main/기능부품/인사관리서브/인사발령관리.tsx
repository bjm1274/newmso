            ...
            <header className="p-4 md:p-5 border-b border-[var(--border)] bg-[var(--card)] shrink-0">
                <div className="flex justify-between items-center">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <button onClick={() => setShowForm(!showForm)} className="px-5 py-2.5 bg-[var(--accent)] text-white text-[11px] font-bold rounded-xl shadow-md hover:opacity-90 transition-all">{showForm ? '취소' : '+ 발령 등록'}</button>
                    </div>
                </div>
                <div className="flex gap-1 mt-4 border-b border-[var(--border)] -mb-5">
                    {(['발령목록', '관보생성'] as const).map(tab => (
                        <button key={tab} onClick={() => setActiveTab(tab)} className={`px-5 py-3 text-[11px] font-bold border-b-2 transition-all ${activeTab === tab ? 'border-[var(--accent)] text-[var(--accent)]' : 'border-transparent text-[var(--toss-gray-3)]'}`}>
            ...
            <div className="flex-1 overflow-y-auto p-4 md:p-5 space-y-4 custom-scrollbar bg-[var(--page-bg)]">
                {showForm && (
                    <form onSubmit={handleSubmit} className="bg-[var(--card)] border border-[var(--border)] rounded-2xl p-4 shadow-sm space-y-4 animate-in slide-in-from-top-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <select value={form.staff_id} onChange={e => handleStaffSelect(e.target.value)} className="px-3 py-2.5 text-[11px] font-bold rounded-xl border border-[var(--border)] bg-[var(--input-bg)] text-[var(--foreground)] outline-none" required>
                                <option value="">직원 선택</option>
                                {filtered.map((s: any) => <option key={s.id} value={s.id}>{s.name} ({s.department || '미배정'} · {s.position || '미정'})</option>)}
                            </select>
                            <select value={form.order_type} onChange={e => setForm({ ...form, order_type: e.target.value })} className="px-3 py-2.5 text-[11px] font-bold rounded-xl border border-[var(--border)] bg-[var(--input-bg)] text-[var(--foreground)] outline-none">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 p-4 bg-[var(--tab-bg)] rounded-xl border border-[var(--border)]">
                            <div><label className="text-[9px] font-bold text-[var(--toss-gray-4)] block mb-1">현재 부서</label><input value={form.before_dept} readOnly className="w-full px-2 py-2 text-[11px] font-bold rounded-lg bg-[var(--tab-bg)] text-[var(--toss-gray-4)] border-none" /></div>
                            <div>
                                <label className="text-[9px] font-bold text-[var(--accent)] block mb-1">변경 부서 →</label>
                                <input list="dept-list" value={form.after_dept} onChange={e => setForm({ ...form, after_dept: e.target.value })} placeholder="변경될 부서 선택/입력" className="w-full px-2 py-2 text-[11px] font-bold rounded-lg border border-[var(--accent)]/30 bg-blue-50/30 text-[var(--foreground)] outline-none placeholder:text-[var(--toss-gray-3)]" />
                            ...
                            <div><label className="text-[9px] font-bold text-[var(--toss-gray-4)] block mb-1">현재 직급</label><input value={form.before_position} readOnly className="w-full px-2 py-2 text-[11px] font-bold rounded-lg bg-[var(--tab-bg)] text-[var(--toss-gray-4)] border-none" /></div>
                            <div>
                                <label className="text-[9px] font-bold text-[var(--accent)] block mb-1">변경 직급 →</label>
                                <input list="position-list" value={form.after_position} onChange={e => setForm({ ...form, after_position: e.target.value })} placeholder="변경될 직급 선택/입력" className="w-full px-2 py-2 text-[11px] font-bold rounded-lg border border-[var(--accent)]/30 bg-blue-50/30 text-[var(--foreground)] outline-none placeholder:text-[var(--toss-gray-3)]" />
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <input type="text" value={form.reason} onChange={e => setForm({ ...form, reason: e.target.value })} placeholder="발령 사유" className="px-3 py-2.5 text-[11px] font-bold rounded-xl border border-[var(--border)] bg-[var(--input-bg)] text-[var(--foreground)] outline-none placeholder:text-[var(--toss-gray-3)]" />
                        ...
                        <div className="flex justify-end"><button type="submit" className="px-4 py-2.5 bg-[var(--accent)] text-white text-[11px] font-bold rounded-xl shadow-md">발령 등록</button></div>
                    </form>
                )}
                ...
                <div className="flex gap-1 bg-[var(--card)] border border-[var(--border)] rounded-xl p-1 w-fit overflow-x-auto">
                    {['전체', ...ORDER_TYPES].map(t => (
                        <button key={t} onClick={() => setFilter(t)} className={`px-3 py-1.5 text-[10px] font-bold rounded-lg transition-all whitespace-nowrap ${filter === t ? 'bg-[var(--accent)] text-white' : 'text-[var(--toss-gray-3)]'}`}>{t}</button>
                    ))}
                </div>
                ...
                {activeTab === '발령목록' && (
                    <div className="bg-[var(--card)] border border-[var(--border)] rounded-2xl overflow-hidden shadow-sm">
                        <table className="w-full text-[11px]">
                            ...
                            <tbody>
                                {filteredRecords.length === 0 ? <tr><td colSpan={5} className="px-4 py-10 text-center text-[var(--toss-gray-3)] font-bold">발령 이력이 없습니다</td></tr> : filteredRecords.map((r: any) => (
                                    <tr key={r.id} className="border-b border-[var(--border)] hover:bg-[var(--muted)]/50">
                                        ...
                                        <td className="px-4 py-3"><span className={`px-2 py-1 rounded-lg text-[10px] font-bold ${r.order_type === '승진' ? 'bg-emerald-100 text-emerald-700' : r.order_type?.includes('전보') ? 'bg-blue-100 text-blue-700' : r.order_type === '휴직' ? 'bg-[var(--tab-bg)] text-[var(--toss-gray-4)]' : 'bg-purple-100 text-purple-700'}`}>{r.order_type}</span></td>
                ...
                {activeTab === '관보생성' && (
                    <div className="bg-[var(--card)] border border-[var(--border)] rounded-2xl p-4 shadow-sm">
                        ...
                        <div className="bg-[var(--tab-bg)] border border-[var(--border)] rounded-xl p-4 font-mono text-[11px] text-[var(--foreground)] whitespace-pre-wrap leading-relaxed">
                            ...
                            {filteredRecords.filter((r: any) => { const d = new Date(r.effective_date); const now = new Date(); return d.getMonth() === now.getMonth() && d.getFullYear() === now.getFullYear(); }).map((r: any, i: number) => (
                                <div key={r.id} className="py-2 border-b border-[var(--border-subtle)]">
                            ...
                            {filteredRecords.filter((r: any) => { const d = new Date(r.effective_date); const now = new Date(); return d.getMonth() === now.getMonth() && d.getFullYear() === now.getFullYear(); }).length === 0 && <p className="text-center text-[var(--toss-gray-3)] py-5">이번 달 발령 내역이 없습니다</p>}
                        </div>
                    </div>
                )}
            </div>
