    ...
    <div className="flex flex-col h-full app-page animate-in fade-in duration-500">
      <div className="p-4 md:p-8 border-b border-[var(--toss-border)] bg-[var(--toss-card)] flex flex-col md:flex-row justify-between items-start md:items-center gap-4 shrink-0">
        <div>
          ...
          <p className="text-[10px] text-[var(--toss-blue)] font-bold mt-1 tracking-widest">연차·휴가 통합 관리 시스템</p>
        </div>
      ...
      <div className="flex-1 p-4 md:p-10 overflow-y-auto custom-scrollbar">
        {activeTab === '연차/휴가 신청내역' && (
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="p-6 bg-[var(--toss-card)] border border-[var(--toss-border)] shadow-sm rounded-[1.5rem] text-center">
                <p className="text-[9px] font-semibold text-[var(--toss-gray-3)] uppercase">승인 대기</p>
                <p className="text-2xl font-semibold text-orange-500 mt-1">{leaves.filter(l => l.status === '대기').length}</p>
              </div>
              <div className="p-6 bg-[var(--toss-card)] border border-[var(--toss-border)] shadow-sm rounded-[1.5rem] text-center">
                <p className="text-[9px] font-semibold text-[var(--toss-gray-3)] uppercase">잔여 연차 (직원별)</p>
                ...
                <p className="text-[9px] text-[var(--toss-gray-3)] mt-1">입사일·사용이력 기반</p>
              </div>
              <div className="p-6 bg-[var(--toss-card)] border border-[var(--toss-border)] shadow-sm rounded-[1.5rem] text-center">
                <p className="text-[9px] font-semibold text-[var(--toss-gray-3)] uppercase">연차 사용</p>
                <p className="text-2xl font-semibold text-[var(--toss-blue)] mt-1">{leaves.filter(l => l.leave_type === '연차' && l.status === '승인').length}</p>
              </div>
              <div className="p-6 bg-[var(--toss-card)] border border-[var(--toss-border)] shadow-sm rounded-[1.5rem] text-center">
                <p className="text-[9px] font-semibold text-[var(--toss-gray-3)] uppercase">기타 휴가</p>
                <p className="text-2xl font-semibold text-purple-600 mt-1">{leaves.filter(l => l.leave_type !== '연차' && l.status === '승인').length}</p>
              </div>
              <div className="p-6 bg-[var(--toss-card)] border border-[var(--toss-border)] shadow-sm rounded-[1.5rem] text-center">
                <p className="text-[9px] font-semibold text-[var(--toss-gray-3)] uppercase">준수율</p>
            ...
            <div className="bg-[var(--toss-card)] border border-[var(--toss-border)] rounded-[2.5rem] overflow-hidden shadow-xl">
              <div className="overflow-x-auto no-scrollbar">
                <table className="w-full text-left border-collapse min-w-[800px]">
                  <thead className="bg-[var(--toss-gray-1)]/50 text-[10px] font-semibold text-[var(--toss-gray-3)] border-b border-[var(--toss-border)] uppercase">
                    <tr>
                      ...
                      <tr key={l.id} className="hover:bg-[var(--toss-blue-light)]/30 transition-all group">
                        <td className="px-8 py-5">
                          <div className="flex flex-col">
                            ...
                            <span className="text-[9px] text-[var(--toss-gray-3)] uppercase">
                        ...
                        <td className="px-8 py-5 text-right">
                          {l.status === '대기' && (
                            <div className="flex justify-end gap-2">
                              <button onClick={() => handleStatusUpdate(l.id, '승인')} className="px-4 py-2 bg-[var(--toss-blue)] text-white text-[10px] font-semibold rounded-xl shadow-lg hover:scale-[0.98] transition-all">승인</button>
                              <button onClick={() => handleStatusUpdate(l.id, '반려')} className="px-4 py-2 bg-[var(--toss-card)] border border-[var(--toss-border)] text-[10px] font-semibold text-[var(--toss-gray-3)] rounded-xl hover:text-red-600 hover:border-red-600 transition-all">반려</button>
                            </div>
                          )}
                        </td>
                      </tr>
        ...
        {activeTab === '연차 자동부여 설정' && (
          <div className="bg-[var(--toss-card)] p-10 border border-[var(--toss-border)] shadow-xl rounded-[2.5rem] text-center max-w-2xl mx-auto">
            ...
            <div className="mt-6">
              <button onClick={runAnnualLeaveAutoGrant} disabled={loading} className="w-full py-4 bg-[var(--toss-blue)] text-white font-semibold rounded-lg text-sm hover:opacity-90 disabled:opacity-50">
                {loading ? '처리 중...' : '📅 입사일 기준 연차 자동 부여 실행'}
              </button>
            </div>
            <div className="mt-10 p-6 bg-[var(--toss-blue-light)] rounded-lg text-left">
              <h4 className="text-[11px] font-semibold text-[var(--toss-blue)] mb-2">💡 연차 산정 기준 안내</h4>
              <p className="text-[10px] text-[var(--toss-blue)] font-bold leading-relaxed">
