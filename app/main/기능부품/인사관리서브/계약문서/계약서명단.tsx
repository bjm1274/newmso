...
export default function ContractList({ selectedCo, staffs, contracts = [], onSelect, checkedIds, setCheckedIds }: any) {
  ...
  const getStatus = (staffId: number) => {
    ...
    if (!contract) return { label: '미요청', class: 'bg-gray-50 text-gray-400 border-gray-100' };
  ...
  return (
    <div className="bg-white border border-gray-100 shadow-sm overflow-hidden flex flex-col h-[800px]">
      <div className="p-6 border-b border-gray-50 bg-gray-50/50 flex justify-between items-center">
        <h3 className="text-[11px] font-semibold text-gray-400 uppercase tracking-widest">계약 대상자 명단</h3>
        <span className="text-[10px] font-bold text-blue-600">필터 결과: {filtered.length}명</span>
      </div>
      <div className="flex-1 overflow-y-auto custom-scrollbar">
        <table className="w-full text-left border-collapse">
          <thead className="bg-white text-[9px] font-semibold text-gray-300 border-b border-gray-100 sticky top-0 z-10 uppercase">
            <tr>
                ...
                <tr key={s.id} onClick={() => onSelect(s.id)} className={`hover:bg-blue-50/20 cursor-pointer transition-colors ${checkedIds.includes(s.id) ? 'bg-blue-50/30' : ''}`}>
                  ...
                  <td className="p-4">
                    <div className="flex flex-col">
                      ...
                      <span className="text-[9px] text-gray-400 font-bold uppercase">{s.company} / {s.department}</span>
                    </div>
                  </td>
                  <td className="p-4">
                  ...
                  <td className="p-4 text-center" onClick={(e) => e.stopPropagation()}>
                    <button 
                      ...
                      className="px-3 py-1 bg-white border border-gray-200 text-[9px] font-semibold text-gray-400 hover:text-blue-600 hover:border-blue-600 transition-all"
                    >
                      재요청
                    </button>
                  </td>
