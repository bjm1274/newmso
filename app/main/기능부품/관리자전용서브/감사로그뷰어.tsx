...
export default function AuditLogViewer() {
  ...
  return (
    <div className="bg-white border border-[var(--toss-border)] rounded-lg overflow-hidden shadow-xl">
      <div className="p-6 border-b border-[var(--toss-border)]">
        <h3 className="text-xl font-semibold text-[var(--foreground)]">감사 로그</h3>
        <p className="text-[10px] text-[var(--toss-gray-3)] font-bold mt-1">급여·결재·인사 등 주요 변경 이력</p>
      </div>
      <div className="max-h-[500px] overflow-y-auto">
        {loading ? (
          ...
          <table className="w-full text-left text-xs">
            <thead className="bg-[var(--toss-gray-1)] text-[10px] font-semibold text-[var(--toss-gray-3)] uppercase">
              <tr>
            ...
            <tbody className="divide-y divide-gray-50">
              {logs.map((l: any) => (
                <tr key={l.id} className="hover:bg-[var(--toss-gray-1)]">
                  <td className="p-4 font-mono text-[10px]">{new Date(l.created_at).toLocaleString()}</td>
