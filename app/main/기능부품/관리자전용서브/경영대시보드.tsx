  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700">
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
        <div className="bg-[var(--toss-card)] p-6 border border-[var(--toss-border)] shadow-sm rounded-lg">
          <p className="text-[9px] font-bold text-[var(--toss-gray-3)] uppercase">월 인건비</p>
          <p className="text-xl font-bold text-[var(--foreground)] mt-1">₩{(metrics.totalLaborCost || 0).toLocaleString()}</p>
        </div>
        <div className="bg-[var(--toss-card)] p-6 border border-[var(--toss-border)] shadow-sm rounded-lg">
          <p className="text-[9px] font-bold text-[var(--toss-gray-3)] uppercase">재고 가치</p>
          <p className="text-xl font-bold text-[var(--foreground)] mt-1">₩{(metrics.inventoryValue || 0).toLocaleString()}</p>
        </div>
        <div className="bg-[var(--toss-card)] p-6 border border-[var(--toss-border)] shadow-sm rounded-lg">
          <p className="text-[9px] font-bold text-[var(--toss-gray-3)] uppercase">결재 대기</p>
          <p className="text-xl font-bold text-red-500 mt-1">{metrics.pendingApprovals ?? 0}건</p>
        </div>
        <div className="bg-[var(--toss-card)] p-6 border border-[var(--toss-border)] shadow-sm rounded-lg">
          <p className="text-[9px] font-bold text-[var(--toss-gray-3)] uppercase">출퇴근률</p>
          <p className="text-xl font-bold text-[var(--toss-blue)] mt-1">{metrics.attendanceRate ?? '-'}%</p>
        </div>
        <div className="bg-[var(--toss-card)] p-6 border border-[var(--toss-border)] shadow-sm rounded-lg">
          <p className="text-[9px] font-bold text-[var(--toss-gray-3)] uppercase">연차 사용률</p>
          <p className="text-xl font-bold text-purple-600 mt-1">{metrics.leaveUsageRate ?? '-'}%</p>
        </div>
        <div className="bg-[var(--toss-card)] p-6 border border-[var(--toss-border)] shadow-sm rounded-lg">
          <p className="text-[9px] font-bold text-[var(--toss-gray-3)] uppercase">효율 지수</p>
          <p className="text-xl font-bold text-green-600 mt-1">{metrics.efficiencyScore ?? '-'}</p>
        </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div className="bg-[var(--toss-card)] border border-[var(--toss-border)] p-8 rounded-lg shadow-sm">
          ...
          <div className="h-40 flex items-end justify-between gap-2">
            {[40, 65, 45, 90, 55, 70, 85].map((h, i) => (
              <div key={i} className="flex-1 flex flex-col items-center gap-2 h-full">
                ...
                <span className="text-[9px] font-bold text-[var(--toss-gray-3)]">{['월','화','수','목','금','토','일'][i]}</span>
              </div>
            ))}
          </div>
        </div>
