/**
 * Extracted rule-aware rotation assignment generator.
 */
import type { RosterGenerationRule } from '@/lib/roster-generation-rules';
import { orderRuleAwareRotationCandidates } from './orderRuleAwareRotationCandidates';
import { selectDistributedDays } from './roster-distributed-days';
import {
...
export function buildRuleAwareRotationAssignments({
  ...
  for (let index = 0; index < days; index += 1) {
    ...
    if (rule.avoidEveningAfterNight && previousBand === 'night') {
      candidates = candidates.filter(
    ...
    const orderedCandidates = orderRuleAwareRotationCandidates({
      bandCounts,
      blockPreference,
      candidates,
    ...
    if (isWeekend) weekendWorkCount += 1;
    if (isHoliday) holidayWorkCount += 1;
  }
  ...
  for (let index = 0; index < days; index += 1) {
    ...
    const dayOfWeek = new Date(`${monthDates[index]}T00:00:00`).getDay();
    if (dayOfWeek === 0 || dayOfWeek === 6) weekendWorkCount = Math.max(0, weekendWorkCount - 1);
    if (holidayDateSet?.has(monthDates[index])) holidayWorkCount = Math.max(0, holidayWorkCount - 1);
    finalWorkStreak = 0;
  }

  if (weekendAssignmentCounts) weekendAssignmentCounts[staffIndex] = weekendWorkCount;
  if (holidayAssignmentCounts) holidayAssignmentCounts[staffIndex] = holidayWorkCount;
