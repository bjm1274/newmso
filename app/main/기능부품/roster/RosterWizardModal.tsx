  ...
  if (isMobile) {
    return (
      ...
      >
        <DesktopOnlyNotice
          feature="근무표 자동편성"
          description="5단계 위저드와 셀 편집이 PC 폭에 최적화되어 있어 모바일에서는 정상 동작하지 않습니다. 가로 768px 이상에서 다시 열어 주세요."
          className="w-full max-w-sm"
          action={
            <button
              type="button"
              onClick={closeWizard}
              className="rounded-[var(--radius-lg)] border border-[var(--border)] bg-[var(--card)] px-5 py-2.5 text-sm font-bold text-[var(--foreground)]"
              aria-label="닫기"
            >
              닫기
            </button>
