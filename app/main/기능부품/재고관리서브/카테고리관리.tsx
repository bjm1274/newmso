...
function CategoryNode({ cat, onEdit, onDelete, onAdd, depth = 0 }: {
  ...
  return (
    <div style={{ marginLeft: depth * 20 }}>
      <div className={`flex items-center justify-between p-2.5 rounded-[var(--radius-md)] mb-1 border ${depth === 0 ? 'border-[var(--border)] bg-[var(--card)]' : 'border-[var(--border)]/50 bg-[var(--muted)]/30'} group`} data-testid={`category-row-${cat.id}`}>
  ...
  return (
    <div className="p-4 md:p-5 space-y-5" data-testid="inventory-category-manager-view">
      ...
      {categories.length === 0 ? (
        ...
        <div className="bg-[var(--card)] border border-[var(--border)] rounded-[var(--radius-lg)] p-4 shadow-sm space-y-1">
