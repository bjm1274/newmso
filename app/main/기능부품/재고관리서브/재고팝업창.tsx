    ...
    <div className="absolute inset-0 bg-black/20 backdrop-blur-sm z-50 flex items-center justify-center p-8" onClick={onClose}>
      <div className="bg-white w-full max-w-md rounded-[2.5rem] p-8 shadow-2xl space-y-4" onClick={e => e.stopPropagation()}>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div><label className="text-xs font-bold text-[var(--toss-gray-3)] ml-1">유효기간 (선택)</label><SmartDatePicker value={newItem.expiration_date} onChange={val => setNewItem({ ...newItem, expiration_date: val })} className="w-full h-[41px] px-3 bg-[var(--toss-gray-1)] rounded-[16px] text-sm font-bold" /></div>
        </div>
    ...
    <div className="absolute inset-0 bg-black/20 backdrop-blur-sm z-50 flex items-center justify-center p-8" onClick={onClose}>
      <div className="bg-white w-full max-w-sm rounded-[2.5rem] p-8 shadow-2xl text-center space-y-4" onClick={e => e.stopPropagation()}>
        ...
        <div className={`p-6 rounded-3xl ${modalData.type === 'in' ? 'bg-blue-50' : 'bg-orange-50'}`}>
          {modalData.type === 'in' && (<div className="space-y-2 mb-4 text-left"><div><label className="text-xs font-bold text-[var(--toss-gray-3)] ml-1">LOT</label><input className="w-full p-2 bg-white rounded-[16px] text-sm font-bold border" value={lotInput} onChange={e => setLotInput(e.target.value)} /></div><div><label className="text-xs font-bold text-[var(--toss-gray-3)] ml-1">유통기한</label><SmartDatePicker value={expInput} onChange={val => setExpInput(val)} className="w-full h-9 px-2 bg-white rounded-[16px] text-sm font-bold border" /></div></div>)}
