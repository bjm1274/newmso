    ...
    <div className="absolute inset-0 bg-black/20 backdrop-blur-sm z-50 flex items-center justify-center p-8" onClick={onClose}>
      <div className="bg-white w-full max-w-md rounded-[2.5rem] p-8 shadow-2xl space-y-4" onClick={e => e.stopPropagation()}>
        ...
        <div className="space-y-2">
           ...
           <input className="w-full p-3 bg-[var(--toss-gray-1)] rounded-[16px] text-sm font-bold" placeholder="품목명" value={newItem.name} onChange={e=>setNewItem({...newItem, name: e.target.value})} />
           <input className="w-full p-3 bg-[var(--toss-gray-1)] rounded-[16px] text-sm font-bold" placeholder="규격" value={newItem.spec} onChange={e=>setNewItem({...newItem, spec: e.target.value})} />
        </div>

        {/* [NEW] 단가 입력 추가 */}
        <div>
            <label className="text-xs font-bold text-[var(--toss-blue)] ml-1">단가 (필수)</label>
            <input type="number" className="w-full p-3 bg-blue-50 border border-blue-100 rounded-[16px] text-sm font-bold" placeholder="개당 가격" value={newItem.price} onChange={e=>setNewItem({...newItem, price: Number(e.target.value)})} />
        </div>

        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="text-xs font-bold text-[var(--toss-gray-3)] ml-1">초기 수량</label>
            <input type="number" className="w-full p-3 bg-[var(--toss-gray-1)] rounded-[16px] text-sm font-bold" value={newItem.quantity} onChange={e=>setNewItem({...newItem, quantity: Number(e.target.value)})} />
          </div>
          <div>
            <label className="text-xs font-bold text-red-400 ml-1">최소 유지</label>
            <input type="number" className="w-full p-3 bg-red-50 text-red-500 rounded-[16px] text-sm font-bold" value={newItem.safety_stock} onChange={e=>setNewItem({...newItem, safety_stock: Number(e.target.value)})} />
          </div>
        </div>

        <div className="grid grid-cols-2 gap-4">
          <div><label className="text-xs font-bold text-[var(--toss-gray-3)] ml-1">바코드 (선택)</label><input className="w-full p-3 bg-[var(--toss-gray-1)] rounded-[16px] text-sm font-bold" value={newItem.barcode} onChange={e=>setNewItem({...newItem, barcode: e.target.value})} /></div>
          <div><label className="text-xs font-bold text-[var(--toss-gray-3)] ml-1">유효기간 (선택)</label><input type="date" className="w-full p-3 bg-[var(--toss-gray-1)] rounded-[16px] text-sm font-bold text-[var(--toss-gray-4)]" value={newItem.expiration_date} onChange={e=>setNewItem({...newItem, expiration_date: e.target.value})} /></div>
        </div>

        <div><label className="text-xs font-bold text-[var(--toss-gray-3)] ml-1">공급사</label><input className="w-full p-3 bg-[var(--toss-gray-1)] rounded-[16px] text-sm font-bold" value={newItem.supplier} onChange={e=>setNewItem({...newItem, supplier: e.target.value})} /></div>
        
        <button onClick={handleAddItem} className="w-full py-4 bg-[var(--toss-blue)] text-white rounded-[12px] font-bold shadow-lg mt-2">등록 완료</button>
      </div>
    </div>
  );
}
    ...
    <div className="absolute inset-0 bg-black/20 backdrop-blur-sm z-50 flex items-center justify-center p-8" onClick={onClose}>
      <div className="bg-white w-full max-w-sm rounded-[2.5rem] p-8 shadow-2xl text-center space-y-4" onClick={e => e.stopPropagation()}>
        ...
        <div className={`p-6 rounded-3xl ${modalData.type === 'in' ? 'bg-blue-50' : 'bg-orange-50'}`}>
          {modalData.type === 'in' && (<div className="space-y-2 mb-4 text-left"><div><label className="text-xs font-bold text-[var(--toss-gray-3)] ml-1">LOT</label><input className="w-full p-2 bg-white rounded-[16px] text-sm font-bold border" value={lotInput} onChange={e=>setLotInput(e.target.value)} /></div><div><label className="text-xs font-bold text-[var(--toss-gray-3)] ml-1">유통기한</label><input type="date" className="w-full p-2 bg-white rounded-[16px] text-sm font-bold border" value={expInput} onChange={e=>setExpInput(e.target.value)} /></div></div>)}
          {modalData.type === 'out' && (<div className="mb-4 text-left"><label className="text-xs font-bold text-[var(--toss-gray-3)] ml-1">사용 부서</label><select value={targetDept} onChange={e=>setTargetDept(e.target.value)} className="w-full p-2 bg-white rounded-[16px] text-sm font-bold border"><option value="">선택...</option>{depts.map((d: any) => <option key={d.id} value={d.id}>{d.name}</option>)}</select></div>)}
        ...
        <button onClick={handleStockProcess} className={`w-full py-4 text-white rounded-[12px] font-bold shadow-lg ${modalData.type === 'in' ? 'bg-[var(--toss-blue)]' : 'bg-orange-500'}`}>확인</button>
      </div>
    </div>
  );
}