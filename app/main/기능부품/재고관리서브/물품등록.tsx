...
export default function ProductRegistration({ user, suppliers, fetchInventory, fetchSuppliers }: any) {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="bg-white p-6 md:p-10 border border-[#E5E8EB] shadow-xl rounded-[2.5rem]">
        <div className="mb-8">
          <h2 className="text-2xl font-bold text-[#191F28] tracking-tighter italic">신규 물품 자산 등록</h2>
          <p className="text-[10px] text-[#3182F6] font-bold mt-1 tracking-widest">재고·자산 정보를 입력해 주세요.</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          ...
          {/* 보험코드 · 규격 */}
          <div className="space-y-2">
            <label className="text-[10px] font-bold text-[#8B95A1] uppercase tracking-widest">보험코드 (선택)</label>
            <input
          ...
          <div className="space-y-2">
            <label className="text-[10px] font-bold text-[#8B95A1] uppercase tracking-widest">규격 (선택)</label>
            <input
          ...
          <div className="space-y-2">
            <label className="text-[10px] font-bold text-[#8B95A1] uppercase tracking-widest">배정 부서 (부서별 현황에 표시)</label>
            <select
          ...
          <div className="flex items-center pt-6">
            <label className="flex items-center gap-3 cursor-pointer group">
              ...
              <span className="text-xs font-bold text-[#4E5968] group-hover:text-[#3182F6] transition-colors">UDI 공급내역 보고 대상</span>
            </label>
          </div>
        </div>

        <button onClick={handleRegisterProduct} disabled={loading} className="w-full py-5 bg-[#3182F6] text-white rounded-[16px] font-bold text-sm shadow-sm hover:scale-[0.98] transition-all">✅ 물품 자산 등록하기</button>
      </div>
    </div>
  );
}
