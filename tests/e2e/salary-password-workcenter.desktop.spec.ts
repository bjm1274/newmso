/**
 * salary-password-workcenter.desktop.spec.ts
 *
 * 워크센터 기준 재작성: 급여명세서 비밀번호 검증.
 *
 * 진입 경로: 내정보(마이페이지) 메뉴 → 급여·증명서 → 월별 정산 카드
 *
 * 주의:
 *   - 급여명세서 비밀번호 기능은 마이페이지(내정보) 메뉴에 위치하며,
 *     인사관리 워크센터와 별개의 경로를 사용한다.
 *   - erp_e2e_workcenter: '1' 플래그를 추가하되, 진입 경로는
 *     원본(/main?open_menu=내정보)과 동일하게 유지한다.
 *   - 급여대장/ModLedger 경유가 아닌 마이페이지 급여명세서 탭을 사용.
 */

import { expect, test } from '@playwright/test';
import { fakeUser, mockSupabase, seedSession } from './helpers';

test('workcenter: salary slip verification submits the password exactly as entered', async ({ page }) => {
  ...
  // 내정보(마이페이지) 진입 경로는 워크센터와 별개이므로 open_menu 경로는 동일하게 유지.
