...
test('admin sidebar walkthrough opens each submenu in order without runtime errors', async ({ page }) => {
  ...
  await openAdminSubMenu(page, '급여이상치');
  await expect(page.getByRole('heading', { name: '급여 이상치 자동 감지' })).toBeVisible();
  ...
  await openAdminSubMenu(page, '공문서대장');
  await expect(page.getByRole('heading', { name: '공문서 발송 대장' })).toBeVisible();
