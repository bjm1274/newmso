/**
 * 마이페이지/연차휴가 a11y AA 회귀 검사
 * - open_mypage_tab=leave 로 진입
...
test('마이페이지/연차휴가 a11y AA @a11y', async ({ page }) => {
  await seedSession(page);
  await page.goto('/main?open_menu=내정보&open_mypage_tab=leave');
  ...
  expect(
    blocking,
    `마이페이지/연차휴가에 critical/serious WCAG AA 위반:\n${formatViolations(blocking)}`,
  ).toHaveLength(0);
});
