/**
 * 마이페이지/급여명세서 a11y AA 회귀 검사
 * - open_mypage_tab=records_salary 로 진입
...
test('마이페이지/급여명세서 a11y AA @a11y', async ({ page }) => {
  await seedSession(page);
  await page.goto('/main?open_menu=내정보&open_mypage_tab=records_salary');
  ...
  expect(
    blocking,
    `마이페이지/급여명세서에 critical/serious WCAG AA 위반:\n${formatViolations(blocking)}`,
  ).toHaveLength(0);
});
