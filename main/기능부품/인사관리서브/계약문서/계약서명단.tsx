            ...
            {filtered.map((s: any) => {
              ...
              return (
                <tr key={s.id} onClick={() => onSelect(s.id)} className={`hover:bg-blue-50/20 cursor-pointer transition-colors ${checkedIds.includes(s.id) ? 'bg-blue-50/30' : ''}`}>
                  ...
                  <td className="p-4">
                    <span className={`px-2 py-0.5 text-[9px] font-black border ${status.class}`}>
                      {status.label}
                    </span>
                  </td>
                  <td className="p-4 text-center" onClick={(e) => e.stopPropagation()}>
                    <button 
                      ...
                      className="px-3 py-1 bg-white border border-gray-200 text-[9px] font-black text-gray-400 hover:text-blue-600 hover:border-blue-600 transition-all"
                    >
                      재요청
                    </button>
                  </td>
